...
이전 버전과의 호환성 유지를 위해 inittab 파일을 유지하기는 하지만, `시스템 동작과는 아무 상관이 없다`. init 프로세스가 실행하는 스크립트 파일은 `/etc/rc.d/init.d` 디렉터리에 위치하며, `/etc/rc*.d` 디렉터리에 있는 파일은 `/etc/rc.d/init.d` 파일에 대한 심볼릭 링크이다. 
