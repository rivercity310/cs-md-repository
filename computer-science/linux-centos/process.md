...
at으로 설정한 작업의 결과는 지정된 파일에 저장되고, 만약 표준 출력을 파일로 전환하지 않았다면 작업 결과가 메일로 전달된다. 이 경우 시스템에 메일이 설정되어 있어야 한다. at으로 생성된 작업 파일은 `/var/spool/at` 디렉터리에 저장된다. 생성된 작업 파일은 작업이 완료되면 자동으로 삭제하고, 디렉터리 `/var/spool/at`은 `root` 사용자만 읽을 수 있다.
